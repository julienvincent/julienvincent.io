...
don't want to be faffing with some unfamiliar pager interface, scrolling endlessly through a wall of text, each file
stacked one after the other, endlessly.
...
jd --from vvsxmnmq --to master@origin -Ss
```
...
OK, so my idea was to hack together a tool which could bridge the gap between Jujutsu and existing Neovim tooling that
already understands git.
...
Some common flows where I might this:
...
- Viewing someones PR, which may be pointing to an outdated trunk or ancestor
  (`jd --from feature@origin --to master@origin -S`)
